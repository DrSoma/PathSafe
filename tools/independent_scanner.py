...
def scan_file(filepath):
    ...
    with open(filepath, "rb") as f:
        ...
        for idx, (ifd_off, entries) in enumerate(chain):
            ...
            for e in entries:
                if e["tag"] == 256:  # ImageWidth
                    ...
                    f.seek(e["value_offset"])
                    struct.unpack(endian + "H", f.read(2))[0]
                elif e["tag"] == 270:
            ...
            if "label" in desc_lower:
                ...
                findings.append(
                    (
                        "LABEL IMAGE PRESENT",
                        f"IFD #{idx} at offset {ifd_off}: {width}x{height} - label IFD still in chain",
                    )
                )
                ...
                findings.append(
                    (
                        "MACRO IMAGE PRESENT",
                        f"IFD #{idx} at offset {ifd_off}: {width}x{height} - macro IFD still in chain",
                    )
                )
            ...
            print(
                f"  IFD #{idx}: offset={ifd_off:,}  {width}x{height}  [{img_type}]{exif_str}{gps_str}"
            )
            if tag270_val:
        ...
        for idx, (_ifd_off, entries) in enumerate(chain):
            for e in entries:
                ...
                if e["tag"] in PHI_SENSITIVE_TAGS:
                    ...
                    if e["tag"] in (306, 36867, 36868):  # DateTime tags
                        if stripped and stripped != "0000:00:00 00:00:00":
                            findings.append(
                                ("DATETIME TAG", f"IFD #{idx} {e['name']}: {stripped[:50]}")
                            )
                    elif e["tag"] == 270:
                        ...
                        for field in ["ScanScope ID=", "User=", "Filename=", "Date=", "Time="]:
                            if field in value:
                                ...
                                if (
                                    fval
                                    and not all(c == "X" for c in fval)
                                    and fval not in ("01/01/00", "00:00:00", "")
                                ):
                        ...
                        findings.append(
                            (
                                "SENSITIVE TAG",
                                f"IFD #{idx} {e['name']}: non-empty ({len(stripped)} bytes)",
                            )
                        )
        print()
        ...
        for idx, (_ifd_off, entries) in enumerate(chain):
            for e in entries:
                if e["tag"] in (
                    ...
                    37510,
                    42016,
                ):  # XMP, IPTC, ICC, UserComment, ImageUniqueID
                    raw = read_tag_bytes(f, e)
                    if raw and raw != b"\x00" * len(raw):
                        tag_name = e["name"]
                        preview = raw[:40].decode("ascii", errors="replace")
                        print(
                            f"  IFD #{idx} {tag_name} ({e['tag']}): {len(raw)} bytes, preview: {preview!r}"
                        )
                        findings.append(
                            ("BINARY PHI TAG", f"IFD #{idx} {tag_name}: {len(raw)} non-zero bytes")
                        )
                    else:
                ...
                if e["tag"] == 270:
                    ...
                    if "label" in desc or "macro" in desc:
                        ...
                        if offset_entry and count_entry:
                            ...
                            f.seek(data_off)
                            sample = f.read(64)
                            is_blank = (sample == b"\x00" * len(sample)) or (
                                sample[:2] == b"\xff\xd8" and b"\x00" * 16 in sample
                            )
                            ...
                            if not is_blank:
                                findings.append(
                                    (
                                        "IMAGE DATA INTACT",
                                        f"IFD #{idx} [{img_type}] still has non-blank image data",
                                    )
                                )
                    break
        print()
