# PathSafe Test Results

## Current Automated Test Suite (v1.1.0)

**Platform:** Linux 6.8.0-101-generic, Python 3.12, pytest
**Total:** 748 automated tests, 0 failures

| Module | Tests | Description |
|--------|-------|-------------|
| test_svs.py | 12 | SVS handler: scan, de-identify, extended PHI fields (Time Zone, ImageID, Left, Top, camera calibration) |
| test_ndpi.py | 19 | NDPI handler: scan, de-identify, private tag sweep, scanprofile (65477), barcode type (65480) |
| test_serializer.py | 44 | Serializer: auto/mapping/template modes, validation, preview, rename plan, manifest, edge cases |
| test_dicom.py | ~80 | DICOM handler: PS3.15 tags, UID remapping, sequence traversal |
| test_scanner*.py | ~60 | PHI pattern detection: accession formats, MRN, SSN, dates |
| test_tiff*.py | ~80 | TIFF parser: BigTIFF, multi-IFD, strips, ICC profiles, EXIF/GPS sub-IFDs |
| test_verify.py | 7 | Post-de-identification verification |
| test_unlink_ifd.py | 8 | IFD unlinking for label/macro removal |
| Other | ~438 | BIF, SCN, MRXS, generic TIFF, adversarial, stress tests, filename PHI |

### Real-Slide CLI Validation

| Test | Format | Mode | Files | Result |
|------|--------|------|-------|--------|
| SVS auto-rename | SVS | --rename auto --prefix DoD | 11 | 11 serialized (DoD_001.svs - DoD_011.svs) |
| SVS mapping | SVS | --rename mapping --mapping-file map.csv | 2 of 11 | 2 matched, 9 skipped correctly |
| SVS template | SVS | --rename template "{prefix}_{date}_{index}" --checksum | 1 | DOD_20260317_0001.svs + SHA-256 in manifest |
| SVS collision | SVS | --rename template "same_name" | 11 | Correctly rejected before any copy |
| NDPI auto | NDPI | --rename auto --prefix CLDN18 | 1 (1.3 GB) | CLDN18_0001.ndpi, 12 PHI cleared |
| SVS default | SVS | (no --rename) | 1 | Original filename preserved, no manifest |

---

## v1.0.0 Manual Integration Test Results
...
### Bug 1: Label/Macro Image False Positive After De-identification (Critical)
- **Affected:** NDPI, SVS
- **Symptom:** `pathsafe verify` reported LabelImage/MacroImage as PHI even after de-identification
...
- **Affected:** NDPI, SVS
- **Symptom:** Running `pathsafe deidentify --in-place` on an already-de-identified file still reported "cleared N finding(s)" instead of "already clean"
- **Root Cause:** `_blank_label_macro()` in the de-identify path didn't check if images were already blanked before overwriting them
...
- **Fix:** Added PN VR check to `_is_dicom_deidentified()`: values consisting only of `^` characters are treated as empty/de-identified
- **Files Modified:** `pathsafe/formats/dicom.py`

### Bug 6: MRXS Copy Mode Missing Companion Data Directory
- **Affected:** MRXS
- **Symptom:** `pathsafe deidentify --output` for MRXS files only copied the `.mrxs` file, not the companion data directory (`slide/`), causing de-identify to find nothing to clean
- **Root Cause:** `deidentify_file()` in `deidentifier.py` used `shutil.copy2()` which only copies single files, not the companion directory MRXS requires
...
- **Files Modified:** `pathsafe/deidentifier.py`

---

## Phase 1: NDPI Format Testing

**Test File:** `sample_slide_001.ndpi` (local, from production batch)

| # | Test | Command | Result | Notes |
|---|------|---------|--------|-------|
| 1 | Info | `pathsafe info <file>` | PASS | Shows format=ndpi, byte_order, page_count, tag details |
| 2 | Scan | `pathsafe scan --verbose <file>` | PASS | Found NDPI_BARCODE "XX-XX-XXXXXX" |
| 3 | De-identify (copy) | `pathsafe deidentify <file> --output <dir>` | PASS | Cleared 2 findings, verified clean |
| 4 | Verify | `pathsafe verify <output>` | PASS | Reports CLEAN |
| 5 | Scan after de-identify | `pathsafe scan --verbose <output>` | PASS | 0 findings, CLEAN |
| 6 | Info after de-identify | `pathsafe info <output>` | PASS | PHI Status: CLEAN |
| 7 | Dry-run | `pathsafe deidentify --dry-run` | PASS | Nothing modified, no output file created |
| 8 | In-place | `pathsafe deidentify --in-place <copy>` | PASS | Before: 1 finding, after: CLEAN |
| 9 | Certificate | `pathsafe deidentify --certificate <path>` | PASS | Valid JSON with UUID, SHA-256, timestamps |
| 10 | Batch (workers) | `pathsafe deidentify --workers 4 <dir>` | PASS | 18/28 files processed (disk space limit) |
| 11 | Already clean | `pathsafe deidentify --in-place <clean>` | PASS | Reports "already clean" (after fix) |

---

## Phase 2: SVS Format Testing

**Test File:** `sample_slide_001.svs`, `sample_slide_002.svs`, `sample_slide_003.svs` (local SVS files)
...
| 3 | De-identify (copy) | `pathsafe deidentify <file> --output <dir>` | PASS | Cleared 7 findings, verified clean |
| 4 | Verify | `pathsafe verify <output>` | PASS | CLEAN |
| 5 | Scan after de-identify | `pathsafe scan --verbose <output>` | PASS | 0 findings |
| 6 | Info after de-identify | `pathsafe info <output>` | PASS | PHI Status: CLEAN |
| 7 | Dry-run | `pathsafe deidentify --dry-run` | PASS | No output file created |
| 8 | In-place | `pathsafe deidentify --in-place <copy>` | PASS | Before: 7 findings, after: CLEAN |
| 9 | Certificate | `pathsafe deidentify --certificate` | PASS | Valid JSON with findings_cleared=7, SHA-256 |
| 10 | Already clean | `pathsafe deidentify --in-place <clean>` | PASS | Reports "already clean" (after fix) |
...
| 3 | De-identify (in-place) | `pathsafe deidentify --in-place <file>` | PASS | Cleared 4 findings, verified clean |
| 4 | Verify | `pathsafe verify <file>` | PASS | CLEAN |
| 5 | Scan after de-identify | `pathsafe scan --verbose <file>` | PASS | 0 findings |
| 6 | Info after de-identify | `pathsafe info <file>` | PASS | PHI Status: CLEAN |
| 7 | Already clean | `pathsafe deidentify --in-place <file>` | PASS | Reports "already clean" |
...
| 3 | De-identify (in-place) | `pathsafe deidentify --in-place DCM_0.dcm` | PASS | Cleared 9 findings, verified clean |
| 4 | Verify | `pathsafe verify DCM_0.dcm` | PASS | CLEAN |
| 5 | Scan after de-identify | `pathsafe scan --verbose DCM_0.dcm` | PASS | 0 findings |
| 6 | Already clean | `pathsafe deidentify --in-place DCM_0.dcm` | PASS | Reports "already clean" |
| 7 | Batch scan | `pathsafe scan --verbose <dir>/` | PASS | 6 files scanned: 1 clean, 5 with PHI (45 findings) |
| 8 | Batch de-identify | `pathsafe deidentify --in-place <dir>/` | PASS | 5 de-identified, 1 already clean, 0 errors. All verified clean. |
...
| 3 | De-identify | `pathsafe deidentify --in-place CMU-1.tiff` | PASS | Correctly reports "already clean" |
...
| 7 | Missing --output/--in-place | `pathsafe deidentify file` | PASS | Error: requires --output or --in-place |
...
| # | Test | Result | Notes |
|---|------|--------|-------|
| 1 | De-identified NDPI readable | PASS | OpenSlide opened 26880x27648, 9 levels, read 256x256 region OK |
| 2 | De-identified SVS readable | PASS | OpenSlide opened 89640x81388, 4 levels, read 256x256 region OK |
| 3 | Generic TIFF readable | PASS | OpenSlide opened 46000x32914, 9 levels, read 256x256 region OK |
| 4 | Parallel vs sequential identical pixels | PASS | Same SVS file de-identified both ways; pixel data matches exactly |
...
| 17 | 6 date de-identification checks | PASS | Zeroed/empty dates detected as de-identified, real dates detected as PHI |
...
| File | Size | Result |
|------|------|--------|
| large_sample_slide.ndpi | 4.5 GB | PASS: info/scan work, found accession XX-XX-XXXXXX, 200 pages |

---

## Test Environment

- **NDPI samples:** Local production batch files (production data, ~600MB each)
- **SVS samples:** Local production SVS files (Aperio AT2 scanner, ~1.6GB each)
...
- **PathSafe install:** pip install -e ".[gui]" at local environment
...
| 2 | CRITICAL | NDPI tag 65449 (SCANNER_PROPS) not scanned | Added scan/de-identify for Created, Updated, NDP.S/N, Macro.S/N keys | PASS |
...
| 12 | MEDIUM | DICOM PatientIdentityRemoved not set after de-identification | Now sets (0012,0062) to "YES" and (0012,0063) to de-identification method | PASS |
...
| 4 | DICOM de-identify + verify (6 files) | PASS | All 6 clean after de-identification |
| 5 | DICOM UID remapping valid | PASS | New UIDs are valid DICOM format, deterministic |
| 6 | DICOM idempotency | PASS | Second de-identify pass clears 0 findings |
| 7 | DICOM PatientIdentityRemoved flag | PASS | Set to "YES" after de-identification |
...
| `pathsafe/formats/ndpi.py` | Added SCANNER_PROPS scanning/de-identifying, extra metadata tags, false-clean fix |
| `pathsafe/formats/svs.py` | Added extra metadata tag scanning/de-identifying |
...
| `pathsafe/formats/generic_tiff.py` | Added extra metadata tag scanning/de-identifying |
...
| `pathsafe/deidentifier.py` | Added MRXS companion directory copy in copy mode |
