...
from pathsafe.formats.ndpi import _NDPI_HANDLED_TAGS, NDPI_PRIVATE_TAG_RANGE, NDPIHandler
...
class TestNDPIScanProfileAndBarcodeType:
    """Regression tests for NDPI_SCANPROFILE (65477) and NDPI_BARCODE_TYPE (65480)."""

    def test_scan_detects_scanprofile(self, handler, tmp_ndpi_with_scanprofile):
        """Tag 65477 (SCANPROFILE) is detected as PHI."""
        result = handler.scan(tmp_ndpi_with_scanprofile)
        assert any(f.tag_id == 65477 for f in result.findings)

    def test_scan_detects_barcode_type(self, handler, tmp_ndpi_with_scanprofile):
        """Tag 65480 (BARCODE_TYPE) is detected as PHI."""
        result = handler.scan(tmp_ndpi_with_scanprofile)
    ...
    def test_not_double_reported(self, handler, tmp_ndpi_with_scanprofile):
        """Tags 65477/65480 appear exactly once each (explicit PHI_TAGS, not sweep)."""
        result = handler.scan(tmp_ndpi_with_scanprofile)
        assert sum(1 for f in result.findings if f.tag_id == 65477) == 1
        assert sum(1 for f in result.findings if f.tag_id == 65480) == 1
