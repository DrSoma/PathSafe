...
class TestGenerateCertificate:
    ...
    def test_integrity_measure_passed(self):
        batch = _make_batch_result(integrity=True)
        cert = generate_certificate(batch)
        integrity_measures = [m for m in cert["measures"] if "integrity" in m["measure"].lower()]
...
class TestGeneratePdfCertificate:
    ...
    def test_missing_required_keys_raises(self, tmp_path):
        ...
        with pytest.raises(ValueError, match="files"):
            generate_pdf_certificate({"certificate_id": "x", "summary": {}}, pdf_path)
        # Missing 'summary' key
        with pytest.raises(ValueError, match="summary"):
            generate_pdf_certificate({"certificate_id": "x", "files": []}, pdf_path)
...
class TestGenerateScanReport:
    ...
    def test_phi_findings_renders_larger(self, tmp_path):
        ...
        phi_data = _make_scan_data(n_files=3, phi_files=2, errors=0, findings_per_phi=5)
    ...
    def test_multi_file_batch(self, tmp_path):
        data = _make_scan_data(n_files=20, phi_files=5, errors=2, findings_per_phi=3)
        pdf_path = tmp_path / "multi.pdf"
        generate_scan_report(data, pdf_path)
        assert pdf_path.exists()
        assert pdf_path.stat().st_size > 500
...
class TestInstitutionInScanReport:
    ...
    def test_institution_renders_larger_pdf(self, tmp_path):
        ...
        generate_scan_report(data, pdf_with, institution="Memorial General Hospital")
    ...
    def test_institution_pdf_valid(self, tmp_path):
        ...
        generate_scan_report(data, pdf_path, institution="University Hospital Zurich")
        assert pdf_path.read_bytes()[:5] == b"%PDF-"
...
class TestInstitutionInCertificate:
    ...
    def test_institution_renders_larger_pdf(self, tmp_path):
        batch = _make_batch_result()
        cert_with = generate_certificate(batch, institution="Memorial Hospital")
        cert_without = generate_certificate(batch)
        pdf_with = tmp_path / "with_inst.pdf"
        pdf_without = tmp_path / "without_inst.pdf"
        generate_pdf_certificate(cert_with, pdf_with, institution="Memorial Hospital")
    ...
    def test_institution_pdf_valid(self, tmp_path):
        batch = _make_batch_result()
        cert = generate_certificate(batch, institution="Johns Hopkins")
        pdf_path = tmp_path / "inst.pdf"
        generate_pdf_certificate(cert, pdf_path, institution="Johns Hopkins")
        assert pdf_path.read_bytes()[:5] == b"%PDF-"

    def test_json_and_pdf_with_institution(self, tmp_path):
        batch = _make_batch_result()
        cert_path = tmp_path / "cert.json"
        generate_certificate(batch, output_path=cert_path, institution="Test Hospital")
        assert cert_path.exists()
