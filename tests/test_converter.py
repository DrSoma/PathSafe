...
def _make_mock_slide(
    ...
    if level_dimensions is None:
        w, h = dimensions
        level_dimensions = tuple((w // (2**i), h // (2**i)) for i in range(level_count))
    ...
    def test_convert_to_pyramidal_tiff(self, tmp_path):
        ...
        with patch.dict("sys.modules", {"openslide": mock_os}):
            ...
            result = conv.convert_file(
                source, output, target_format="tiff", tile_size=256, quality=85
            )
...
class TestTimestampReset:
    ...
    def test_reset_timestamps_on_convert(self, tmp_path):
        ...
        with patch.dict("sys.modules", {"openslide": mock_os}):
            ...
            conv._openslide = None
            result = conv.convert_file(source, output, target_format="png", reset_timestamps=True)
    ...
    def test_reset_timestamps_on_error_does_not_crash(self, tmp_path):
        ...
        with patch.dict("sys.modules", {"openslide": mock_os}):
            ...
            conv._openslide = None
            result = conv.convert_file(source, output, target_format="tiff", reset_timestamps=True)
...
class TestConvertCLI:
    ...
    def test_convert_single_file(self, runner, tmp_path):
        ...
        with patch.dict("sys.modules", {"openslide": mock_os}):
            ...
            result = runner.invoke(main, ["convert", str(source), "-o", str(output), "-t", "png"])
    ...
    def test_convert_extract_label(self, runner, tmp_path):
        ...
        with patch.dict("sys.modules", {"openslide": mock_os}):
            ...
            result = runner.invoke(
                main, ["convert", str(source), "-o", str(output), "--extract", "label"]
            )
    ...
    @pytest.mark.skipif(
        not _has_tifffile(), reason="tifffile not installed (optional convert dependency)"
    )
    def test_convert_batch_directory(self, runner, tmp_path):
        ...
        with patch.dict("sys.modules", {"openslide": mock_os}):
            ...
            result = runner.invoke(
                main, ["convert", str(input_dir), "-o", str(output_dir), "-t", "tiff"]
            )
    ...
    def test_convert_with_reset_timestamps(self, runner, tmp_path):
        ...
        with patch.dict("sys.modules", {"openslide": mock_os}):
            ...
            result = runner.invoke(
                main, ["convert", str(source), "-o", str(output), "-t", "png", "--reset-timestamps"]
            )
    ...
    def test_extract_requires_file(self, runner, tmp_path):
        ...
        result = runner.invoke(
            main, ["convert", str(input_dir), "-o", str(output), "--extract", "label"]
        )
...
class TestMissingDeps:
    ...
    def test_missing_openslide_message(self):
        ...
        with (
            patch.dict("sys.modules", {"openslide": None}),
            pytest.raises(ImportError, match="openslide-python"),
        ):
    ...
    def test_missing_tifffile_message(self):
        ...
        with (
            patch.dict("sys.modules", {"tifffile": None}),
            pytest.raises(ImportError, match="tifffile"),
        ):
            conv._tifffile = None
    ...
    def test_missing_numpy_message(self):
        ...
        with patch.dict("sys.modules", {"numpy": None}), pytest.raises(ImportError, match="numpy"):
            conv._numpy = None
            conv._require_numpy()
