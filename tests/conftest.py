...
def tmp_svs(tmp_path):
    ...
    desc = (
        ...
        b"|Date = 06/15/24"
        b"|Time = 10:30:00"
        b"|Time Zone = GMT-05:00"
        b"|User = jdoe@hospital.org"
        b"|MPP = 0.2520"
        b"|Left = 22.946301"
        b"|Top = 17.375731"
        b"|LineCameraSkew = -0.000222"
        b"|LineAreaXOffset = 0.008968"
        b"|LineAreaYOffset = -0.003024"
        b"|ImageID = 1030624"
        b"\x00"
    )
    entries = [
...
@pytest.fixture
def tmp_ndpi_with_scanprofile(tmp_path):
    """NDPI file with tag 65477 (SCANPROFILE) and tag 65480 (BARCODE_TYPE) PHI."""
    barcode = b"AS-24-123456\x00"
    scan_profile = (
        b'<?xml version="1.0" encoding="utf-8"?>\r\n'
        b"<Profile>\r\n"
        b"  <rescansetting><NumOfFocusPoint>9</NumOfFocusPoint></rescansetting>\r\n"
        b"</Profile>\x00"
    )
    barcode_type = b"DataMatrix,CODE-39,CODE-128,CODE-2-of-5 Interleaved\x00"
    ...
    content = build_tiff(entries)
    filepath = tmp_path / "scanprofile_test.ndpi"
    filepath.write_bytes(content)
    return filepath
...
def build_tiff_multi_ifd_with_strips(ifd_specs, endian="<"):
    ...
    # Build each IFD
    for i, (n, ool_size, _strip_size, tag_entries, strip_data) in enumerate(ifd_infos):
        data_start = ifd_starts[i] + 2 + 12 * n + 4
