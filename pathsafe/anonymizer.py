...
def auto_workers() -> int:
    ...
    WSI anonymization is I/O-bound (large file reads/writes), so more workers
...
def anonymize_file(
    ...
    # anonymized data through a symlink to an unintended location, and
    ...
    # Copy mode: copy to a staging file first, anonymize there, then
    # atomically rename to the final name.  This prevents Ctrl+C or a
    # crash from leaving a partially-anonymized (PHI-leaking) file at
    ...
    if mode == "copy":
        ...
        try:
            ...
            # anonymization; it will be renamed alongside the .mrxs file later.
    ...
    try:
        findings = handler.anonymize(work_path)
    ...
    # Post-anonymization phases: wrap in try/except to clean up staging on failure
    try:
    ...
    # anonymization did not modify any bytes, reuse the copy-time hash
    ...
    # final target path never contains un-anonymized data.
...
def anonymize_batch(
    ...
    progress_callback: Callable[[int, int, Path, AnonymizationResult], None] | None = None,
    workers: int = 1,
    reset_timestamps: bool = False,
    verify_integrity: bool = False,
...
def _batch_sequential(
    ...
    verify: bool,
    dry_run: bool,
    progress_callback: Callable[[int, int, Path, AnonymizationResult], None] | None,
    batch: BatchResult,
    reset_timestamps: bool = False,
    verify_integrity: bool = False,
    stop_check: Callable[[], bool] | None = None,
