...
Ported from proven production code that successfully processed thousands of NDPI files.
"""
...
TAG_NAMES: dict[int, str] = {
    ...
    65423: "NDPI_YOFFSET",
    65424: "NDPI_ZOFFSET",
    65425: "NDPI_UNKNOWN_65425",
    65426: "NDPI_JPEGQUALITY",
    65427: "NDPI_REFERENCE",
    65428: "NDPI_IMGSIZE",
    65429: "NDPI_UNKNOWN_65429",
    65432: "NDPI_UNKNOWN_65432",
    65433: "NDPI_UNKNOWN_65433",
    65439: "NDPI_FOCUSPOINTS",
    65440: "NDPI_UNKNOWN_65440",
    65441: "NDPI_CAPTUREMODE",
    65442: "NDPI_SERIAL_NUMBER",
    65449: "NDPI_SCANNER_PROPS",
    65457: "NDPI_UNKNOWN_65457",
    65458: "NDPI_UNKNOWN_65458",
    65459: "NDPI_UNKNOWN_65459",
    65468: "NDPI_BARCODE",
    65469: "NDPI_UNKNOWN_65469",
    65476: "NDPI_UNKNOWN_65476",
    65477: "NDPI_SCANPROFILE",
    65478: "NDPI_UNKNOWN_65478",
...
class IFDEntry:
    ...
    __slots__ = ("tag_id", "dtype", "count", "value_offset", "entry_offset", "is_inline")
...
class TIFFHeader:
    ...
    def __init__(self, endian: str, is_bigtiff: bool, first_ifd_offset: int) -> None:
        self.endian = endian
        self.is_bigtiff = is_bigtiff
        self.first_ifd_offset = first_ifd_offset


def read_header(f: BinaryIO) -> TIFFHeader | None:
...
def read_ifd(f: BinaryIO, header: TIFFHeader, ifd_offset: int) -> tuple[list[IFDEntry], int]:
    ...
    if header.is_bigtiff:
        ...
        if num_entries > MAX_IFD_ENTRIES:
            logger.debug(
                "IFD at offset %d has %d entries (> %d), likely corrupt",
                ifd_offset,
                num_entries,
                MAX_IFD_ENTRIES,
            )
    ...
    for _e in range(num_entries):
        ...
        entries.append(IFDEntry(tag_id, dtype, count, value_offset, entry_offset, is_inline))
...
def read_tag_numeric(
    f: BinaryIO, header: TIFFHeader, entry: IFDEntry
...
def find_tag_in_ifd(
    f: BinaryIO, header: TIFFHeader, ifd_offset: int, target_tag: int
) -> IFDEntry | None:
...
def find_tag_in_first_ifd(filepath: str, target_tag: int) -> tuple[int | None, int | None]:
...
def iter_ifds(
    f: BinaryIO, header: TIFFHeader, max_pages: int = 500
...
def get_all_string_tags(
    f: BinaryIO, header: TIFFHeader, ifd_offset: int
...
def read_tag_long_array(f: BinaryIO, header: TIFFHeader, entry: IFDEntry) -> list[int]:
