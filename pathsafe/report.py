...
def _mask_path_for_certificate(filepath: Path) -> str:
    """Return only the filename stem for certificate records.

    Directory paths can contain institutional folder names, user home
    directories, or network share names that leak organizational PHI.
    If the filename itself contains PHI patterns (e.g., accession
    numbers), mask it to ``[PHI_FILENAME]``.
    """
    stem = filepath.stem
    if scan_filename_for_phi(filepath):
        return "[PHI_FILENAME]"
    return stem
...
def generate_certificate(
    ...
    for result in batch_result.results:
        record = {
            "filename": _mask_path_for_certificate(result.output_path) + result.output_path.suffix,
            "source_path": _mask_path_for_certificate(result.source_path),
            "output_path": _mask_path_for_certificate(result.output_path),
    ...
    integrity_verified = sum(1 for r in batch_result.results if r.image_integrity_verified is True)
    integrity_failed = sum(1 for r in batch_result.results if r.image_integrity_verified is False)
    ...
    if output_path is not None:
        ...
        if pdf:
            ...
            generate_pdf_certificate(certificate, pdf_path, institution=institution)
...
def _pdf_label_value(pdf: FPDF, label: str, value: str) -> None:
...
def _pdf_kv_table(pdf: FPDF, rows: list[tuple[str, str]]) -> None:
    ...
    for i, (key, value) in enumerate(rows):
        ...
        pdf.cell(col_w[0], 7, key, border=0, fill=fill, new_x="RIGHT", new_y="TOP")
        pdf.set_font("Helvetica", "", 9)
        pdf.cell(col_w[1], 7, str(value), border=0, fill=fill, new_x="LMARGIN", new_y="NEXT")
    pdf.ln(3)


def _pdf_measures_table(pdf: FPDF, measures: list[dict[str, str]]) -> None:
    ...
    pdf.cell(col_w[0], 7, "Measure", border=0, fill=True, new_x="RIGHT", new_y="TOP")
    pdf.cell(col_w[1], 7, "Status", border=0, fill=True, new_x="LMARGIN", new_y="NEXT")
    ...
    for i, m in enumerate(measures):
        if i % 2 == 0:
            ...
            fill = False
        pdf.set_font("Helvetica", "", 9)
        pdf.cell(col_w[0], 7, m["measure"], border=0, fill=fill, new_x="RIGHT", new_y="TOP")
        status = m["status"]
        r, g, b = _STATUS_COLORS.get(status, (0, 0, 0))
        pdf.set_text_color(r, g, b)
        pdf.set_font("Helvetica", "B", 9)
        pdf.cell(col_w[1], 7, status.upper(), border=0, fill=fill, new_x="LMARGIN", new_y="NEXT")
        pdf.set_text_color(0, 0, 0)
    pdf.ln(3)


def _pdf_file_results_table(pdf: FPDF, files: list[dict[str, object]]) -> None:
    """Render the file results table with dynamic columns.

    Integrity and SHA-256 columns are only shown when those features were used.
    """
    # Determine which optional columns are needed
    has_integrity = any(frec.get("image_integrity_verified") is not None for frec in files)
    has_sha = any(frec.get("sha256_after") for frec in files)

    # Build column spec dynamically
    columns = [
        ("#", 8),
        ("Filename", 52 if not has_sha else 42),
        ("Format", 18),
        ("Cleared", 18),
        ("Verified", 18),
    ]
    if has_integrity:
        columns.append(("Integrity", 20))
    if has_sha:
        columns.append(("SHA-256", 66))

    # Redistribute extra width to Filename if fewer columns
    headers = [c[0] for c in columns]
    col_w = [c[1] for c in columns]
    ...
    for i, frec in enumerate(files):
        ...
        max_fn_len = 36 if not has_sha else 28
        row_vals = [
            str(i + 1),
            _trunc(frec.get("filename", ""), max_fn_len),
            frec.get("format", "?"),
        ...
        if has_sha:
            ...
            row_vals.append(_trunc(sha, 40))

        for j, val in enumerate(row_vals):
            nx = "RIGHT" if j < len(row_vals) - 1 else "LMARGIN"
            ny = "TOP" if j < len(row_vals) - 1 else "NEXT"
            pdf.cell(col_w[j], 5.5, val, border=0, fill=fill, new_x=nx, new_y=ny)
    pdf.ln(3)


def generate_pdf_certificate(
    certificate: dict[str, Any], output_path: Path, institution: str = ""
) -> Path:
    ...
    if missing:
        raise ValueError(f"Certificate dict is missing required keys: {', '.join(sorted(missing))}")
    ...
    pdf.cell(
        0, 5, f"PathSafe v{certificate.get('pathsafe_version', '?')}", new_x="LMARGIN", new_y="NEXT"
    )
    pdf.set_text_color(0, 0, 0)
    ...
    if files:
        ...
        if files_with_findings:
            ...
            for frec in files_with_findings:
                ...
                pdf.cell(0, 6, _trunc(frec.get("filename", ""), 80), new_x="LMARGIN", new_y="NEXT")
                pdf.set_text_color(0, 0, 0)
                ...
                for finding in frec["findings"]:
                    ...
                    preview = _sanitize_for_pdf(_trunc(finding.get("value_preview", ""), 55))
                    ...
                    pdf.cell(0, 4, f"Replaced with: {replacement}", new_x="LMARGIN", new_y="NEXT")
                    pdf.set_text_color(0, 0, 0)
...
def _pdf_scan_file_table(pdf: FPDF, results: list[dict[str, object]]) -> None:
    """Render the file results table for a scan report (dynamic columns)."""
    has_sha = any(entry.get("sha256") for entry in results)
    if has_sha:
        col_w = [8, 42, 20, 18, 102]  # total = 190
        headers = ["#", "Filename", "Status", "Findings", "SHA-256 (before)"]
    else:
        col_w = [8, 62, 24, 96]  # total = 190, wider filename + findings
        headers = ["#", "Filename", "Status", "Findings"]
    ...
    for i, entry in enumerate(results):
        ...
        if error:
            ...
            status = "PHI FOUND"
            visible = [f for f in findings if f.get("tag_name", "") not in _PDF_HIDDEN_TAGS]
            findings_str = f"{len(visible)} finding(s)"

        # Row number
        pdf.set_font("Helvetica", "", 7)
        fn_max = 40 if has_sha else 48
        pdf.cell(col_w[0], 5.5, str(i + 1), border=0, fill=fill, new_x="RIGHT", new_y="TOP")
        # Filename
        pdf.cell(
            col_w[1],
            5.5,
            _trunc(filepath.name, fn_max),
            border=0,
            fill=fill,
            new_x="RIGHT",
            new_y="TOP",
        ...
        pdf.set_text_color(r, g, b)
        pdf.set_font("Helvetica", "B", 7)
        pdf.cell(col_w[2], 5.5, status, border=0, fill=fill, new_x="RIGHT", new_y="TOP")
        pdf.set_text_color(0, 0, 0)
        # Findings summary
        pdf.set_font("Helvetica", "", 7)
        if has_sha:
            pdf.cell(col_w[3], 5.5, findings_str, border=0, fill=fill, new_x="RIGHT", new_y="TOP")
            # SHA-256
            pdf.cell(
                col_w[4], 5.5, sha256 or "-", border=0, fill=fill, new_x="LMARGIN", new_y="NEXT"
            )
        else:
            pdf.cell(
                col_w[3], 5.5, findings_str, border=0, fill=fill, new_x="LMARGIN", new_y="NEXT"
            )
    pdf.ln(3)
...
_LEGEND_ENTRIES = [
    # -- Identifiers --
    ("Barcode", "Accession number or specimen ID encoded in the slide barcode area."),
    (
        "Reference",
    ...
    ("Scan Profile", "Scanner configuration profile name (may reveal institutional workflows)."),
    ("Barcode Type", "Type of barcode detected on the slide (e.g., DataMatrix, CODE-128)."),
    # -- Embedded metadata --
    (
        "ICC Color Profile",
...
def _pdf_legend(pdf: FPDF, used_tags: set[str] | None = None) -> None:
    ...
    for i, (name, description) in enumerate(_LEGEND_ENTRIES):
        ...
        pdf.get_x()
        pdf.get_y()
        pdf.cell(50, 5, name, border=0, fill=fill, new_x="RIGHT", new_y="TOP")

        # Description column (wrapped)
        pdf.set_font("Helvetica", "", 8)
        pdf.multi_cell(130, 5, description, border=0, fill=fill, new_x="LMARGIN", new_y="NEXT")
...
def generate_scan_report(
    ...
    pdf.cell(
        0,
        5,
        f"PathSafe v{pathsafe.__version__}  |  "
    ...
    # --- Detailed findings ---
    phi_results = [
        r for r in results if not r.get("is_clean") and not r.get("error") and r.get("findings")
    ]
    if phi_results:
        pdf.set_font("Helvetica", "B", 11)
        pdf.cell(0, 7, "Detailed Findings", new_x="LMARGIN", new_y="NEXT")
        ...
        for entry in phi_results:
            ...
            pdf.set_font("Helvetica", "B", 9)
            pdf.set_text_color(200, 130, 0)
            pdf.cell(0, 6, _trunc(filepath.name, 80), new_x="LMARGIN", new_y="NEXT")
            pdf.set_text_color(0, 0, 0)
