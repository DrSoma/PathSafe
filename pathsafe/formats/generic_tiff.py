...
class GenericTIFFHandler(TiffFormatHandler):
    ...
    def can_handle(self, filepath: Path) -> bool:
        ...
        try:
            ...
            logger.warning("TIFF magic byte check failed for %s: %s", filepath, e)
            return False
                ...
                for _ifd_offset, entries in iter_ifds(f, header):
                    for entry in entries:
                        ...
                        if entry.dtype == 2:  # ASCII string tags
                            ...
                            for char_off, length, matched, _label in str_findings:
                                findings.append(
                                    PHIFinding(
                ...
                for entry in entries:
                    ...
                    if entry.dtype == 2:  # ASCII string tags
                        ...
                        if str_findings:
                            ...
                            f.write(replacement)
                            for _, _, matched, _label in str_findings:
                                cleared.append(
                                    PHIFinding(
                                        offset=entry.value_offset,
                                        length=entry.total_size,
