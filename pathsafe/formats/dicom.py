...
try:
    ...
    from pydicom.uid import generate_uid  # noqa: F401
...
# Random per-session salt for UID remapping -- avoids using filepath (which
# may contain PHI) as the hash salt.  Generated once at import time so UIDs
# remain deterministic within a single session.
_UID_SALT: str = secrets.token_hex(16)
...
class DICOMHandler(FormatHandler):
    ...
    def get_format_info(self, filepath: Path) -> dict[str, Any]:
        ...
        try:
            ...
            info["is_wsi"] = str(getattr(ds, "SOPClassUID", "")) == WSI_SOP_CLASS_UID
            ...
            info["private_tag_count"] = sum(1 for elem in ds if elem.tag.is_private)
...
def _is_dicom_deidentified(value: str, vr: str) -> bool:
    ...
    return bool(vr == "PN" and all(c == "^" for c in value))


def _remap_uid(original_uid: str) -> str:
    """Generate a deterministic replacement UID from the original.

    Uses SHA-256 hash of original UID + a random per-session salt to produce
    a consistent (within session) but non-reversible replacement.
    The result is a valid DICOM UID (digits and dots only, max 64 chars,
    no component with leading zeros).
    """
    hash_input = f"{original_uid}:{_UID_SALT}".encode()
...
def _scan_sequences(ds: Any, depth: int = 0) -> list[PHIFinding]:
