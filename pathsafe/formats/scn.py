...
# Pre-compiled regex patterns for SCN XML element and attribute scanning.
# Compiled once at import time instead of per-IFD inside the hot loop.
_SCN_ELEM_SCAN_PATTERNS = {
    elem: re.compile(rf"<{elem}[^>]*>([^<]+)</{elem}>", re.IGNORECASE) for elem in SCN_PHI_ELEMENTS
}
_SCN_ATTR_SCAN_PATTERNS = {
    elem: re.compile(rf'{elem}\s*=\s*"([^"]*)"', re.IGNORECASE) for elem in SCN_PHI_ELEMENTS
}
_SCN_ELEM_ANON_PATTERNS = {
    elem: re.compile(rf"(<{elem}[^>]*>)([^<]+)(</{elem}>)", re.IGNORECASE)
    for elem in SCN_PHI_ELEMENTS
}
_SCN_ATTR_ANON_PATTERNS = {
    elem: re.compile(rf'({elem}\s*=\s*")([^"]*?)(")', re.IGNORECASE) for elem in SCN_PHI_ELEMENTS
}
                ...
                for entry in entries:
                    if entry.tag_id == 270 and entry.value_offset not in seen:
                        ...
                        for elem_name in SCN_PHI_ELEMENTS:
                            ...
                            for m in _SCN_ATTR_SCAN_PATTERNS[elem_name].finditer(xml_text):
                ...
                for entry in entries:
                    if entry.tag_id == 270 and entry.value_offset not in seen:
                        ...
                        for elem_name in SCN_PHI_ELEMENTS:
                            ...
                            new_text, count = _SCN_ELEM_ANON_PATTERNS[elem_name].subn(
                                _replace_elem, xml_text
                            )
                            ...
                            def _replace_attr(m: re.Match[str], xml_text: str = xml_text) -> str:
                                # Skip matches inside XML processing instructions (<?...?>)
                                prefix = xml_text[: m.start()]
                                if prefix.rfind("<?") > prefix.rfind("?>"):
                                    return m.group(0)
                            ...
                            new_text, count = _SCN_ATTR_ANON_PATTERNS[elem_name].subn(
                                _replace_attr, xml_text
                            )
                            if count > 0 and new_text != xml_text:
                                xml_text = new_text
                                modified = True
