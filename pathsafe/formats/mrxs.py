...
class MRXSHandler(FormatHandler):
    ...
    def scan(self, filepath: Path) -> ScanResult:
        ...
        try:
            ...
            if slidedat.exists():
                ...
                findings += self._scan_slidedat(config)
                findings += self._scan_slidedat_all_sections(config)
                findings += self._scan_associated_images(config, data_dir)
    ...
    def get_format_info(self, filepath: Path) -> dict[str, Any]:
        ...
        if slidedat.exists():
            try:
                ...
                info["total_data_size"] = sum(f.stat().st_size for f in dat_files)

                # Report associated images
                images = _find_associated_images(config)
                if images:
                    info["associated_images"] = [img_type for img_type, _, _, _ in images]
    ...
    def _scan_slidedat(self, config: configparser.ConfigParser) -> list[PHIFinding]:
    ...
    def _scan_slidedat_all_sections(self, config: configparser.ConfigParser) -> list[PHIFinding]:
    ...
    def _scan_associated_images(
        self, config: configparser.ConfigParser, data_dir: Path
    ) -> list[PHIFinding]:
        """Detect label, macro, and thumbnail images that may contain PHI."""
        findings = []
        images = _find_associated_images(config)

        for img_type, dat_file_key, section_name, _layer_name in images:
    ...
    def _anonymize_slidedat_regex(self, slidedat_path: Path) -> list[PHIFinding]:
        ...
        config and would overwrite any prior byte-level patches.
        """
        cleared = []
        data = slidedat_path.read_bytes()
        raw_findings = scan_bytes_for_phi(data)
...
def _get_data_dir(filepath: Path) -> Path | None:
...
def _write_slidedat(slidedat_path: Path, config: configparser.ConfigParser) -> None:
...
def _is_anonymized(field: str, value: str) -> bool:
    ...
    if all(c == "X" for c in value):
        return True
    return bool(field in DATE_FIELDS and value == "19000101000000")
...
def _resolve_dat_file(
    config: configparser.ConfigParser, dat_file_key: str | None, data_dir: Path
) -> Path | None:
    ...
    # Path traversal protection: ensure resolved path stays within data_dir
    resolved = dat_path.resolve()
    if not resolved.is_relative_to(data_dir.resolve()):
        logger.warning("Skipping suspicious path outside data directory: %s", dat_file_key)
        return None
    if dat_path.exists():
        return dat_path

    # Try as a section reference
    if config.has_section("DATAFILE") and config.has_option("DATAFILE", dat_file_key):
        filename = config.get("DATAFILE", dat_file_key)
        dat_path = data_dir / filename
        resolved = dat_path.resolve()
        if not resolved.is_relative_to(data_dir.resolve()):
            logger.warning("Skipping suspicious path outside data directory: %s", filename)
            return None
        if dat_path.exists():
            return dat_path
...
def _get_image_data_size(config: configparser.ConfigParser, section_name: str) -> int:
...
def _get_image_offset_size(
    config: configparser.ConfigParser, section_name: str, dat_path: Path
) -> tuple[int, int]:
